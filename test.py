...
def test(opt, seg, gmm, alias):
    ...
    gauss = tgm.image.GaussianBlur((15, 15), (3, 3)).cuda()  # Move gauss filter to CUDA device
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            img_agnostic = inputs['img_agnostic'].cuda()  # Move to CUDA device
            parse_agnostic = inputs['parse_agnostic'].cuda()  # Move to CUDA device
            pose = inputs['pose'].cuda()  # Move to CUDA device
            c = inputs['cloth']['unpaired'].cuda()  # Move to CUDA device
            cm = inputs['cloth_mask']['unpaired'].cuda()  # Move to CUDA device
            ...
            save_images(output_cpu, unpaired_names, os.path.join(opt.save_dir, opt.name))
